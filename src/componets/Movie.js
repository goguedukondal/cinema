        ...
        <>
            <Container sx={{ height: "auto" }}>
                <ImageList sx={{ height: "auto", width: "100", display: "flex", justifyContent: "center", flexWrap: "wrap" }}>
                    ...
                    {movie_detail.map((item,idx) => (
                        <ImageListItem  scroll="hidden" key={item.img} sx={{ marginBottom: 3, marginTop: 2, height: 100, overflow: "hidden", border: "1px solid black", borderRadius: 3, boxShadow: "1px 1px 7px rgba(3,6,9,0.8)" }} >
                            <img
                                src={item.img}
                                srcSet={item.img}
                                alt={item.title}
                                        ...
                                        <CardHeader
                                            ...
                                            title={
                                                <Stack spacing={1} style={{ display: "flex", height: 150, width: "auto", justifyContent: "flex-start", alignItems: "flex-start" }}>
                                                    <Typography variant='h5' >{item.movie_name}</Typography>
                                                    <Typography style={{color:"rgba(173,216,230,0.8)"}}  >{item.year},{item.actor_name}</Typography>
                                                    <Stack direction="row" spacing={2} >
                                                        <Typography style={{ width:"auto",paddingLeft:2,paddingRight:6, boxShadow: "1px 1px 3px rgba(200,200,200,0.7)", borderRadius: 5 }} >{item.time}</Typography>
